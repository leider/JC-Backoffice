...
describe('DatumUhrzeit', () => {
  describe('creation', () => {
    ...
    it('kann mit jsDate erzeugt werden', () => {
      expect(DatumUhrzeit.forJSDate(festesDatum).toLocalDateTimeString).to.eql(
        '30.04.2019, 13:43:30'
      );
    ...
    it('kann mit ISO String erzeugt werden', () => {
      expect(
        DatumUhrzeit.forISOString('2019-03-01T00:00:00+01:00')
  ...
  describe('plus und minus (Datum) mit immutability', () => {
    ...
    it('rechnet mit Monaten und Tagen', () => {
      ...
      expect(maerz04.minus({ monate: 1, tage: 4 }).value).to.eql(
        januar31.value
      );
  ...
  describe('plus und minus (Uhrzeit)', () => {
    ...
    it('rechnet mit Minuten', () => {
      expect(vorSommerzeit.plus({ minuten: 10 }).value).to.eql(
        plus10Minuten.value
      );
      expect(plus10Minuten.minus({ minuten: 10 }).value).to.eql(
        vorSommerzeit.value
      );
    });

    it('rechnet mit Stunden', () => {
      expect(vorSommerzeit.plus({ stunden: 1 }).value).to.eql(
        plus1Stunde.value
      );
      expect(vorSommerzeit.plus({ stunden: 4 }).value).to.eql(
        plus4Stunden.value
      );
      expect(plus1Stunde.minus({ stunden: 1 }).value).to.eql(
        vorSommerzeit.value
      );
    });
  });
  ...
  describe('Spezialmethoden', () => {
    it('findet vorhergehenden oder aktuellen ungeraden Monat', () => {
      expect(
        DatumUhrzeit.forISOString('2020-04-30')
          .vorigerOderAktuellerUngeraderMonat.value
      ).to.eql(DatumUhrzeit.forISOString('2020-03-30').value);
      expect(
        DatumUhrzeit.forISOString('2020-03-20')
          .vorigerOderAktuellerUngeraderMonat.value
      ).to.eql(DatumUhrzeit.forISOString('2020-03-20').value);
      expect(
        DatumUhrzeit.forISOString('2020-02-29')
          .vorigerOderAktuellerUngeraderMonat.value
      ).to.eql(DatumUhrzeit.forISOString('2020-01-29').value);
      expect(
        DatumUhrzeit.forISOString('2020-01-20')
          .vorigerOderAktuellerUngeraderMonat.value
      ).to.eql(DatumUhrzeit.forISOString('2020-01-20').value);
      expect(
        DatumUhrzeit.forISOString('2020-12-31')
          .vorigerOderAktuellerUngeraderMonat.value
      ).to.eql(DatumUhrzeit.forISOString('2020-11-30').value);
      expect(
        DatumUhrzeit.forISOString('2020-11-20')
          .vorigerOderAktuellerUngeraderMonat.value
      ).to.eql(DatumUhrzeit.forISOString('2020-11-20').value);
    });

    it('findet mächsten ungeraden Monat', () => {
      expect(
        DatumUhrzeit.forISOString('2020-04-30').naechsterUngeraderMonat.value
      ).to.eql(DatumUhrzeit.forISOString('2020-05-30').value);
      expect(
        DatumUhrzeit.forISOString('2020-03-31').naechsterUngeraderMonat.value
      ).to.eql(DatumUhrzeit.forISOString('2020-05-31').value);
      expect(
        DatumUhrzeit.forISOString('2020-02-29').naechsterUngeraderMonat.value
      ).to.eql(DatumUhrzeit.forISOString('2020-03-29').value);
      expect(
        DatumUhrzeit.forISOString('2020-01-31').naechsterUngeraderMonat.value
      ).to.eql(DatumUhrzeit.forISOString('2020-03-31').value);
      expect(
        DatumUhrzeit.forISOString('2020-12-31').naechsterUngeraderMonat.value
      ).to.eql(DatumUhrzeit.forISOString('2021-01-31').value);
      expect(
        DatumUhrzeit.forISOString('2020-11-30').naechsterUngeraderMonat.value
      ).to.eql(DatumUhrzeit.forISOString('2021-01-30').value);
    });
  ...
  describe('getSet', () => {
    ...
    it('setzen der felder', () => {
      ...
      expect(januar01.setUhrzeit(5, 23).value).to.eql(
        DatumUhrzeit.forISOString('2019-01-01T05:23:00').value
      );
    });
  });
});
