...
const firstLine = 'id;veranstalter;freitext;title;beginn;beschreibung;url;ort;pool_unitref;bild';

function csvLineFor(veranstaltung) {
  const presse = veranstaltung.presse();
  const kopf = veranstaltung.kopf();
  const titel = kopf.titel();
  const beginn = veranstaltung.startMoment().format('DD.MM.YY hh:mm');
  ...
  return ';' + veranstalter + ';' + freitext + ';' + titel + ';' + beginn + ';' + beschreibung + ';' + url + ';' + idVonOrt + ';1444;' + bild + '\r\n';
}
