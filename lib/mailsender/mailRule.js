class MailRule {
  ...
  shouldSend(dateAsMoment) {
    ...
    if (rule === rules[2] || rule === rules[5]) { // 'Am 5. den Folgemonat' oder 'Am 5. zwei Folgemonate'
      return dateAsMoment.date() === 5;
    }
    if (rule === rules[3]) { // 'Am 20. den Folgemonat ab 15.'
      return dateAsMoment.date() === 20;
    }
    if (rule === rules[4]) { // 'Montags die Folgewoche ab Freitag'
  ...
  startAndEndDay(dateAsMoment) {
    ...
    if (rule === rules[5]) { // 'Am 5. zwei Folgemonate'
      start.add(1, 'month').date(1);
