...
${kaputteVeranstaltungen.map(veranst => veranst.presseTemplateInternal()).join('\n\n---\n')}`;
    ...
    store.byDateRangeInAscendingOrder(start, maxDay, (err1, veranstaltungen) => {
      if (err1) {return;}
...
---
${kaputteVeranstaltungen.map(veranst => `<a href="${misc.toFullQualifiedUrl('veranstaltungen', encodeURIComponent(veranst.url()))}">` +
      `${veranst.kopf().titel()} am ${veranst.datumForDisplayShort()} ${veranst.kopf().presseIn()}</a>`).join('\n\n---\n')}
...
function checkFluegel(callback) {
  if (new DatumUhrzeit().wochentag() !== 7) { // Sonntag
    return callback();
  }
...
---
${veranstaltungenMitFluegel.map(veranst => veranst.kopf().titel() + ' am ' + veranst.datumForDisplayShort() + ' ' + veranst.kopf().presseIn()).join('\n\n---\n')}`;
