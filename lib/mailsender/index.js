...
app.post('/send', (req, res, next) => {
  ...
  optionenService.optionen((err, optionen) => {
    ...
    store.zukuenftige((err1, veranstaltungen) => {
      ...
      const markdownToSend = req.body.markdown + selected.map(veranst => veranst.presseTextForMail()).join('\n---\n');
      const message = new Message({subject: 'Test', markdown: markdownToSend});
