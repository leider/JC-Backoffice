...
class OptionValues {
  constructor(object) {
    ...
    ['plakatNum', 'flyerNum', 'plakatGroesse', 'plakatVon', 'hotelpreise', 'genres'].forEach(field => {
      this.state[field] = object[field] || [];
    });
    ['typen', 'kooperationen', 'backlineJazzclub', 'backlineRockshop', 'artists'].forEach(field => {
  ...
  fillFromUI(object) {
    ['typen', 'kooperationen',
      'backlineJazzclub', 'backlineRockshop', 'plakatNum', 'flyerNum',
      'plakatGroesse', 'plakatVon', 'artists', 'hotelpreise', 'genres'].forEach(field => {
  ...
  preisprofile() {
    return [
      {name: 'Kooperation', regulaer: 0, rabattErmaessigt: 0, rabattMitglied: 0},
  ...
  updateHotelpreise(hotel, unterkunft) {
    if (!this.hotels().find(h => h.name === hotel.name)) { // kein Hotel gefunden
      return;
    }
