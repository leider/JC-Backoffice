...
function renderVertrag(language, req, res, next) {
  ...
  store.getVeranstaltung(req.params.url, (err1, veranstaltung) => {
    ...
    app.render(language, {veranstaltung, datum: moment()}, htmlToPdfAndOut(res, next));
  });
}

app.get('/:url/deutsch', (req, res, next) => {
  renderVertrag('deutsch', req, res, next);
});

app.get('/:url/englisch', (req, res, next) => {
  renderVertrag('englisch', req, res, next);
});
