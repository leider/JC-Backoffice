...
module.exports = {
  ...
  pageRename: function pageRename(subdir, pageNameOld, pageNameNew, user, callback) {
    const completePageNameOld = subdir + '/' + pageNameOld + '.md';
    const completePageNameNew = subdir + '/' + pageNameNew + '.md';
    Git.mv(completePageNameOld, completePageNameNew, 'rename: "' + pageNameOld + '" -> "' + pageNameNew + '"', user, err => {
      callback(err, null);
    });
  },
    ...
    Fs.exists(Git.absPath(subdir), function (exists) {
      ...
      Fs.writeFile(pageFile, body.content, err => {
        ...
        Git.log(completePageName, 'HEAD', 1, (ignoredErr, metadata) => {
          ...
          Git.add(completePageName, (body.comment.length === 0 ? 'no comment' : body.comment), user, err1 => {
