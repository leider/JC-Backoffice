...
app.get('/versions/:subdir/:page', (req, res, next) => {
  ...
  wikiService.pageHistory(
    completePageName,
    (err: Error | null, metadata: Metadata[]) => {
      if (err || !metadata) {
        return next(err);
      }
...
app.get('/compare/:subdir/:page/:revisions', (req, res, next) => {
  ...
  wikiService.pageCompare(
    completePageName,
    revisions,
    (err: Error | null, diff: Diff) => {
      if (err || !diff) {
        return next();
      }
...
app.get('/edit/:subdir/:page', (req, res, next) => {
  ...
  const subdir = req.params.subdir;
  const completePageName = subdir + '/' + pageName;
  wikiService.pageEdit(
    completePageName,
    (err: Error | null, content: string, metadata: Metadata[]) => {
      if (err) {
        return next(err);
      }
...
app.post('/:subdir/:page', (req, res) => {
  ...
  wikiService.pageSave(
    ...
    pageName,
    req.body,
    (req.user as User).asGitAuthor,
    (err: Error | null, conflict: boolean) => {
      if (err) {
        statusmessage
