...
app.post('/:subdir/:page', (req, res) => {
  const pageName = renderer.normalize(req.params.page);
  const subdir = req.params.subdir;
  wikiService.pageSave(subdir, pageName, req.body, req.user, (err, conflict) => {
    if (err) {
      statusmessage.errorMessage('Problem', 'Das Speichern war nicht erfolgreich.').putIntoSession(req);
    } else if (conflict) {
      statusmessage.errorMessage('Konflikt', 'Deine Änderungen wurden gespeichert. Du hast allerdings Änderungen eines anderen Benutzers überschrieben. Bitte überprüfe nochmals die Wikiseite.').putIntoSession(req);
    } else {
...
app.post('/rename/:subdir/:page', (req, res, next) => {
  ...
  const subdir = req.params.subdir;
  wikiService.pageRename(subdir, req.params.page, pageNameNew, req.user, err => {
    if (err) {
      statusmessage.errorMessage('Problem', 'Das Speichern war nicht erfolgreich.').putIntoSession(req);
      return next(err);
    }
