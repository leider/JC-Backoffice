...
function createCSV(nachmeldung: boolean, events: Array<Veranstaltung>) {
...
function createResult(
  ...
  const functionToCall =
    selector === 'vergangene' ? store.vergangene : store.zukuenftige;
...
app.get('/', (req, res, next) => {
  store.zukuenftige((err: Error | null, zukuenftige: Array<Veranstaltung>) => {
    ...
    store.vergangene((err1: Error | null, vergangene: Array<Veranstaltung>) => {
      if (err1) {
        return next(err1);
      }
      res.render('choose', {
