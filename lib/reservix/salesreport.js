...
class Salesreport {
  ...
  static forRawResult(reservixID, result) {
    ...
    return Salesreport.forOneResult(reservixID, result.tsServer, result.data[0]);
  }

  static forOneResult(reservixID, tsServer, object) {
    if (reservixID.toString() !== object.eventId.toString()) {
      return new Salesreport();
    }
    object.tsServer = tsServer;
    object.id = (object.eventId && object.eventId.toString());
    object.datumUhrzeit = object.startDate + ' ' + object.startTime;
