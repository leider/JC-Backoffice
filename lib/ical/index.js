...
app.get('/eventsForCalendar', (req, res, next) => {
  const start = new DatumUhrzeit(req.query.start).toJSDate();
  const end = new DatumUhrzeit(req.query.end).toJSDate();
  icalService.termineAsEventsBetween(start, end, (err, events) => {
    if (err) { return next(err); }
...
app.get('/eventsFromIcalURL/:url', (req, res, next) => {
  ...
  icalService.termineFromIcalURL(url, (err, events) => {
    if (err) { return next(err); }
    res.end(JSON.stringify(events));
  });
});
...
app.get('/delete/:id', (req, res, next) => {
  terminstore.remove(req.params.id, (err) => {
    if (err) { return next(err); }
