...
function eventsToObject(contents) {
  if (!contents) { return {}; }
  ...
  function lineToObject(line) {
    function dates(element) {
      function toDate(dayMonthString, stunde = '00:00') {
        ...
        return DatumUhrzeit.forGermanString(dayMonth + ' ' + stunde).toJSDate();
      }
...
Hier eine automatische Erinnerungsmail, dass Deine Aufgabe "${this.event.was}" bis zum ${this.start().format('LL')} erledigt sein soll.
...
class Kalender {
  ...
  eventsToSend(aDatumUhrzeit) {
    const events = eventsToObject(this.text())
      .filter(e => !!e.email)
