...
class Veranstaltung {
  constructor(object) {
    ...
    ['agentur', 'artist', 'eintrittspreise', 'hotel', 'kasse', 'kopf', 'kosten', 'presse',
      'staff', 'unterkunft', 'vertrag', 'salesrep'].forEach(field => {
  ...
  fillFromUI(object) {
    ...
    ['agentur', 'artist', 'eintrittspreise', 'hotel', 'kasse', 'kopf', 'kosten', 'presse',
      'staff', 'unterkunft', 'vertrag'].forEach(field => {
      if (object[field]) {
        this[field]().fillFromUI(object[field]);
      }
    });
    this.state.brauchtHotel = object.brauchtHotel === 'on';
    return this;
  }
  ...
  dealAbsolutEUR() {
    const deal = this.bruttoUeberschussEUR() * this.kosten().dealAlsFaktor();
