...
class Veranstaltung {
  ...
  fillFromUI(object) {
    ...
    if (object.kopf) {
      if (object.startDate) {
        ...
        this.state.startDate = DatumUhrzeit.forGermanString(object.startDate, object.startTime).toJSDate();
        this.state.endDate = DatumUhrzeit.forGermanString(object.endDate, object.endTime).toJSDate();
  ...
  unterkunft() {
    return new Unterkunft(this.state.unterkunft, this.startDatumUhrzeit(), this.artist().name());
  }
...
**Eintritt:** ${this.eintrittspreise().alsPressetext(this.kopf().isKooperation() ? this.kopf().kooperation() : null)}

`;
  }

  presseTemplateInternal() { // für interne Mails
