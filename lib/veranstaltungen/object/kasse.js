...
class Kasse {
  ...
  updateReservix(salesreport) {
    this.state.anzahlBesucherReservix = salesreport.anzahlRegulaer();
    this.state.reservixNettoEUR = salesreport.nettoUmsatz();
    this.state.reservixBruttoEUR = salesreport.bruttoUmsatz();
  }
  ...
  freigabeErfolgtDurch(name) {
    ...
    this.state.kassenfreigabeAm = moment().toDate();
  }
  ...
  freigabeDisplayDatum() {
    return moment(this.state.kassenfreigabeAm).format('LL');
  }
