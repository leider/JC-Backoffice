...
class Eintrittspreise {
  ...
  reservixTypes() {
    return ['regulaer', 'ermaessigt', 'mitglied'];
  }
  ...
  reservixNettoEUR() {
    if (this.reservixEUR() <= 0) {
      return 0;
    }
    const ohneSteuer = this.reservixEUR() / 1.19;
    return (ohneSteuer - config.get('reservix-gebuehr')) * (1 - config.get('reservix-vvk')); // gebuehr in EUR an reservix und vvk an VVK-Stelle
  }
