...
app.post('/submitWithImage', (req, res, next) => {
  new Form().parse(req, (err, fields, files) => {
    ...
    if (dateiname) {
      ...
      copyFile(pfad, path.join(uploadDir, dateiname), errC => {
        if (errC) { return next(err); }
        store.getVeranstaltungForId(fields.id[0], (err1, veranstaltung) => {
          if (err1) { return next(err1); }
          veranstaltung.presse().fillFromUI({
            text: fields['presse[text]'],
            image: dateiname,
            checked: fields['presse[checked]']
          });
      ...
      store.getVeranstaltungForId(fields.id[0], (err1, veranstaltung) => {
        if (err1) { return next(err1); }
        veranstaltung.presse().fillFromUI({text: fields['presse[text]'], checked: fields['presse[checked]']});
        store.saveVeranstaltung(veranstaltung, err2 => {
          if (err2) { return next(err2); }
          statusmessage.successMessage('Gespeichert', 'Deine Änderungen wurden gespeichert').putIntoSession(req);
          res.redirect(fields.returnTo[0]);
  ...
  new Form().parse(req, (err, fields, files) => {
    ...
    if (dateiname) {
      ...
      copyFile(pfad, path.join((istPressefoto ? uploadDir : filesDir), dateiname), errC => {
        if (errC) { return res.send({error: err}); }
        store.getVeranstaltungForId(fields.id[0], (err1, veranstaltung) => {
