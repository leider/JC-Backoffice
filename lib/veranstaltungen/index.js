...
app.get('/', (req, res) => res.redirect('/teamseite'));
...
app.get('/:url', (req, res, next) => {
  if (!res.locals.accessrights.isOrgaTeam()) {
    return res.redirect(req.params.url + '/preview');
  }
...
app.get('/:url/hotel', (req, res, next) => {
  ...
  optionenService.optionen((err, optionen) => {
    ...
    store.getVeranstaltung(req.params.url, (err1, veranstaltung) => {
      ...
      res.render('edit/hotel', {veranstaltung: veranstaltung, optionen: optionen, hotelMailReceiver: hotelMailReceiver});
    });
  });
});
