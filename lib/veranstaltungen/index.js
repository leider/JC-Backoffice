...
app.get('/monat/:monat', (req, res, next) => {
  ...
  store.byDateRangeInAscendingOrder(start, end, (err, result) => {
    if (err) { return next(err); }
    const strings = result.map(veranstaltung => {
      return veranstaltung.kopf().titel() + ' ' + veranstaltung.startMoment().format('ddd DD MMM YY') + ' ' + veranstaltung.kopf().ort();
    });
    res.render('monatsliste', {liste: result});
