...
function veranstaltungenForDisplay(fetcher, next, res, titel) {
  return fetcher((err, veranstaltungen) => {
    ...
    res.render('liste', {titel: titel, veranstaltungen: filterUnbestaetigteFuerJedermann(veranstaltungen, res)});
  });
}
...
app.get('/:url', (req, res, next) => {
  store.getVeranstaltung(req.params.url, (err, veranstaltung) => {
    ...
    if (veranstaltung.reservixID()) {
      return reservixService.salesreportFor(veranstaltung.reservixID(), salesreport => {
        res.render('overview', {veranstaltung, salesreport});
      });
    }
    res.render('overview', {veranstaltung, salesreport: new Salesreport()});
  });
});
