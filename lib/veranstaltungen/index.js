  ...
  function asCalendarEvent(veranstaltung) {
    return {
      ...
      className: (!veranstaltung.kopf().confirmed() ? 'color-geplant ' : '') +
        'verySmall color-' + fieldHelpers.cssColorCode(veranstaltung.kopf().eventTyp())
    };
  }
...
app.get('/new', (req, res, next) => {
  ...
  optionenService.optionenUndOrte((err, optionen, orte) => {
    if (err) { return next(err); }
    res.render('edit/allgemeines', {veranstaltung: new Veranstaltung(), optionen, orte, Vertrag});
  });
});
...
app.get('/imgzip/:monat', (req, res, next) => {
  ...
  store.byDateRangeInAscendingOrder(start, end, (err, result) => {
    ...
    const filename = 'Jazzclub Bilder ' + start.format('MMM \'YY') + '.zip';
    ...
    async.forEachSeries(images,
      (file, cb) => {zip.entry(fs.createReadStream(file.path), {name: file.name}, cb);},
      (err1) => {
        if (err1) {return next(err1);}
        zip.finalize();
      });
