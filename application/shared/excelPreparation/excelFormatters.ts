...
export function createExcelData({ konzert, optionen }: { konzert: Konzert; optionen: OptionValues }) {
  const kasse = konzert.kasse;
  const kosten = konzert.kosten;
  const staff = konzert.staff;
  const kalk = new KonzertKalkulation(konzert);
  const klavierStimmerStandard = konzert.technik.fluegel ? optionen.preisKlavierstimmer : 0;
  ...
  const fluegelStimmerRow = { Art: "Flügelstimmer", Einnahme: "", Ausgabe: kosten.fluegelstimmerEUR || klavierStimmerStandard };
...
export function createExcelDataVermietung({ vermietung, optionen }: { vermietung: Vermietung; optionen: OptionValues }) {
  const kosten = vermietung.kosten;
  const klavierStimmerStandard = vermietung.technik.fluegel ? optionen.preisKlavierstimmer : 0;
